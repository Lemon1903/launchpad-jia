...
import JobDescription from "@/lib/components/CareerComponents/new/JobDescription";
...
export default function ManageCareerPage() {
  ...
  const { timelineStages, interviewsInProgress, dropped, hired, setAndSortCandidates } =
    useCareerApplicants({
      "CV Review": {
        candidates: [],
        droppedCandidates: [],
      ...
      if (response.data.length > 0) {
        ...
        for (const interview of response.data) {
          ...
          if (
            ...
            (interview.currentStep === "CV Screening" && interview.status === "For AI Interview")
          ) {
            if (interview.status === "For Interview" || interview.status === "For AI Interview") {
              isDropped
                ? newTimelineStages["Pending AI Interview"].droppedCandidates.push(interview)
                : newTimelineStages["Pending AI Interview"].candidates.push(interview);
              continue;
            }

            isDropped
              ? newTimelineStages["AI Interview Review"].droppedCandidates.push(interview)
              : newTimelineStages["AI Interview Review"].candidates.push(interview);
            continue;
          }
          ...
          if (interview.currentStep === "Human Interview") {
            if (interview.status === "For Human Interview") {
              isDropped
                ? newTimelineStages["For Human Interview"].droppedCandidates.push(interview)
                : newTimelineStages["For Human Interview"].candidates.push(interview);
              continue;
            }
            if (interview.status === "For Human Interview Review") {
              isDropped
                ? newTimelineStages["Human Interview Review"].droppedCandidates.push(interview)
                : newTimelineStages["Human Interview Review"].candidates.push(interview);
              continue;
            }
          }
          ...
          if (interview.currentStep === "Job Interview") {
            isDropped
              ? newTimelineStages["Pending Job Interview"].droppedCandidates.push(interview)
              : newTimelineStages["Pending Job Interview"].candidates.push(interview);
            continue;
          }

          if (interview.currentStep === "Job Offered") {
            isDropped
              ? newTimelineStages["Job Offered"].droppedCandidates.push(interview)
          ...
          if (interview.currentStep === "Contract Signed") {
            isDropped
              ? newTimelineStages["Contract Signed"].droppedCandidates.push(interview)
  ...
  useEffect(() => {
    const fetchCareer = async () => {
      ...
      try {
        const response = await getCareer(slug as string);
  ...
  const handleCandidateAnalysisComplete = (updatedCandidate: any) => {
    ...
    ].candidates.map((c: any) => (c._id === updatedCandidate._id ? updatedCandidate : c));
  ...
  const dragEndorsedCandidate = (candidateId: string, fromStageKey: string, toStageKey: string) => {
    const candidateIndex = (timelineStages?.[fromStageKey]?.candidates as any[]).findIndex(
  ...
  const handleCandidateAction = async (action: string) => {
    ...
    if (action === "endorse") {
      ...
      try {
        ...
        if (!draggedCandidateRef.current) {
          const updatedStages = { ...timelineStages };
          updatedStages[stage].candidates = updatedStages[stage].candidates.filter(
        ...
        candidateActionToast(
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <span
                style={{ fontSize: 14, color: "#717680", fontWeight: 500, whiteSpace: "nowrap" }}
              >
    ...
    if (action === "reconsider") {
      ...
      try {
        ...
        if (timelineStages?.[stage]) {
          const updatedStages = { ...timelineStages };
          updatedStages[stage].droppedCandidates = updatedStages[stage].droppedCandidates.filter(
            (c: any) => c._id !== selectedCandidate._id,
          );
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <span
                style={{ fontSize: 14, color: "#717680", fontWeight: 500, whiteSpace: "nowrap" }}
              >
                You have approved <strong>{selectedCandidate?.name}'s</strong> request to retake
                interview.
              </span>
            </div>
          </div>,
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <span
                style={{ fontSize: 14, color: "#717680", fontWeight: 500, whiteSpace: "nowrap" }}
              >
                You have rejected <strong>{selectedCandidate?.name}'s</strong> request to retake
                interview.
              </span>
            </div>
          </div>,
    ...
    if (!action && draggedCandidateRef.current) {
      ...
      const newCandidateIndex = (revertedStages?.[toStage]?.candidates as any[]).findIndex(
        (c) => c._id.toString() === selectedCandidate._id,
      );
  ...
  return (
    <>
      {/* Header */}
      <HeaderBar activeLink="Careers" currentPage={career?.jobTitle || ""} icon="la la-suitcase" />
      <div className="container-fluid mt--7" style={{ paddingTop: "6rem" }}>
        ...
        <div style={{ padding: "16px 0 48px", background: "#FDFDFD", minHeight: "100vh" }}>
          ...
          {activeTab === "job-description" && career && (
            // <JobDescription
            //   formData={formData}
            ...
            // />
            <JobDescription {...career} onEditClick={(stepIndex) => {}} />
          )}
          {candidateMenuOpen && (
            <CandidateMenu
              handleCandidateMenuOpen={handleCandidateMenuOpen}
          ...
          {candidateCVOpen && (
            <CandidateCV candidate={selectedCandidateCV} setShowCandidateCV={setCandidateCVOpen} />
          )}
